...
public class Main extends Application {
			...
			public void handle(MouseEvent e) {
				...
				if (!flagMode & !minefield[y][x].isFlagged()) {
					...
					//empty
						...
						//run for loop for all cells around and check for empty cells
							// disable each empty square/edge number
							// if hit a numbered cell, stop the loop
						for(int i = (y - 1); i < (y + 2); i ++) {
							for(int j = (x - 1); j < (x + 2); j ++) {
								try {
									if(minefield[i][j].bombsNear() != 0) {
										btnField[i][j].setText(Integer.toString(minefield[i][j].bombsNear()));
										btnField[i][j].setDisable(true);
										// maybe have an setChecked()&isChecked() to make sure loop doesn't run infinitely
									}
									else {
										btnField[i][j].setDisable(true);
									}
	...
	//TODO create a function for the for loops we use repeatedly so that we can just call that instead (clean up code)
	//TODO better labels & placement, images instead of letters
	//TODO timer and second window for losing/winning games
